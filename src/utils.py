...
def get_train_ops(
  ...
  grads = tf.gradients(loss, tf_variables)   # loss对tf_variables求导
  grad_norm = tf.global_norm(grads)     # 计算梯度的全局范数

  grad_norms = {}    # 保存单个梯度的范数
  ...
  train_op = opt.apply_gradients(
    zip(grads, tf_variables), global_step=train_step)   # 函数会对global_step做自增操作
