...
class MicroChild(Model):
  ...
  def _model(self, images, is_training, reuse=False):
    """
    Compute the logits given the images. 输出尺寸为(N,10)
    model有8个layer
    """
    ...
    with tf.variable_scope(self.name, reuse=reuse):
      ...
      for layer_id in range(self.num_layers + 2):      # model有8个layer
  ...
  def _enas_cell(self, x, curr_cell, prev_cell, op_id, out_filters):
    ...
    x: 所要连接的node的state
    curr_cell:当前的cell id, 即 node id
    prev_cell:所要连接的cell id, 即 node id
    op_id:选择的操作, 一共有5个候选
    ...
    with tf.variable_scope("avg_pool"):
      ...
      avg_pool_c = self._get_C(avg_pool)   # 得到C通道数
  ...
  def _enas_layer(self, layer_id, prev_layers, arc, out_filters):
    """
    Args:
      layer_id: 当前layer的 id
      prev_layers: cache of previous layers. for skip connections    前两层layer的输出, 用作当前 layer 的 node 1 和 node 2
      start_idx: where to start looking at. technically, we can infer this
        from layer_id, but why bother...
    每层有（num_cells+2）个node, node 1 和 node2已确定, cell_0就是node 3
    """

    assert len(prev_layers) == 2, "need exactly 2 inputs"
    layers = [prev_layers[0], prev_layers[1]]
    layers = self._maybe_calibrate_size(layers, out_filters, is_training=True)   # 把通道数变为out_filters
    used = []
    for cell_id in range(self.num_cells):
      prev_layers = tf.stack(layers, axis=0)  # 始终更新保存着所有previous node的状态
      with tf.variable_scope("cell_{0}".format(cell_id)):   # cell_0其实就是node 3
        with tf.variable_scope("x"):
          x_id = arc[4 * cell_id]  # 连接哪个node , 即 index_1
          x_op = arc[4 * cell_id + 1]  # 采用什么操作, 即 op_1
          x = prev_layers[x_id, :, :, :, :]
          x = self._enas_cell(x, cell_id, x_id, x_op, out_filters)
          x_used = tf.one_hot(x_id, depth=self.num_cells + 2, dtype=tf.int32)

        with tf.variable_scope("y"):
          y_id = arc[4 * cell_id + 2]   # 连接哪个node , 即 index_2
          y_op = arc[4 * cell_id + 3]   # 采用什么操作, 即 op_2
          y = prev_layers[y_id, :, :, :, :]
        ...
        layers.append(out)  # layers有7个(num_cells+2)
    ...
    out = tf.gather(out, indices, axis=0)   # 挑出所有的输出node的state进行concat
  ...
  def connect_controller(self, controller_model):
    """
    从controller_model中得到采样序列, 然后建立模型
    """
    if self.fixed_arc is None:
      self.normal_arc, self.reduce_arc = controller_model.sample_arc  # 得到sample的结果（连接哪个node,进行什么操作）
    else:
