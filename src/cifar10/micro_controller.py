...
class MicroController(Controller):
  def __init__(self,
    ...
    self.sample_arc = (arc_seq_1, arc_seq_2)     # 分别对应 (conv cell的采样序列, reduction cell的采样序列)
  ...
  def _build_sampler(self, prev_c=None, prev_h=None, use_bias=False):
    ...
    # 共有(num_cells + 2)个node, 需对num_cells个node采样
    anchors = tf.TensorArray(
      tf.float32, size=self.num_cells + 2, clear_after_read=False)
    anchors_w_1 = tf.TensorArray(
      tf.float32, size=self.num_cells + 2, clear_after_read=False)
    arc_seq = tf.TensorArray(tf.int32, size=self.num_cells * 4)  # nodes 1, 2 are input nodes, so no decisions are needed. 只需要对num_cells个node采样, 每个node包括（index_1, op_1, index_2, op_2）
    if prev_c is None:
